...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!videoPattern.test(args[0]) && playlistPattern.test(args[0])) {
      embed.setAuthor("I am Unable To Play Playlist");
      return message.channel.send(embed);
    }
    ...
    if (urlcheck) {
      try {
        ...
        if (message.include === "copyright") {
          return message
            .reply("This song/video includes **copyrighted** music.")
            .catch(console.error);
        } else {
          console.error(error);
        }
      ...
      try {
        ...
        console.log(error);
        return message.channel.send("Fatal Error | Dm Jamesiboi#0001 For Assistance");
      }
    }
